...
import { admin, apiKey, organization, phoneNumber } from 'better-auth/plugins';
...
import { provisionStore } from './aurora';
...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    // Migration build: email/password works without an email provider (Resend not configured yet).
  ...
  user: {
    ...
    additionalFields: {
      ...
      // Bridge to the Aislely domain (tenant + public store slug)
      aisleyTenantId: {
        type: 'string',
        required: false,
      },
  ...
  databaseHooks: {
    ...
    user: {
      create: {
        before: async (user) => {
          // Bridge: provision an Aislely tenant + store for the new owner.
          try {
            const { tenantId, slug } = await provisionStore(user.name || user.email);
            return { data: { ...user, aisleyTenantId: tenantId, storeSlug: slug } };
          } catch (e) {
            console.error('provisionStore failed:', e);
            return { data: user };
          }
        },
        after: async (user) => {
  ...
  plugins: [
    ...
    // Multi-tenant: each store = an organization
    organization(),
    // Phone-number register/login. DEV: OTP logged to console (no SMS provider yet);
    // swap sendOTP to Twilio/etc. when a provider key is added.
    phoneNumber({
